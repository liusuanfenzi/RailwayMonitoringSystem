...
class StayDetector:
    ...
    def __init__(self, stay_threshold=10, movement_threshold=20, min_frames=10,
                 roi_manager=None, alert_dir="alerts/person_vehicle_detection"):
        """
        初始化停留检测器
        ...
        self.alerted_objects = set()  # 曾经触发过报警的对象
        self.current_staying = set()  # 当前正在停留的对象

        print(
            f"✅ 停留检测器初始化完成 - 阈值: {stay_threshold}秒, 移动阈值: {movement_threshold}像素")
    ...
    def update(self, tracked_objects, timestamp, frame=None):
        ...
        """
        current_ids = set()
        self.current_staying.clear()  # 清空当前停留对象
            ...
            if len(obj) >= 5:
                try:
                    ...
                    if not self._check_in_any_roi(center_x, center_y):
                        # 目标离开ROI，重置状态（但保留报警记录）
                        self._reset_track_state(track_id, keep_alerted=True)
                        continue
                    ...
                    if is_stationary:
                        ...
                        # 计算静止时间
                        stationary_duration = timestamp - \
                            self.stationary_start_time[track_id]

                        # 只有连续静止足够多帧才认为是真正的停留
                        if (self.stationary_frames[track_id] >= self.min_frames and
                                stationary_duration >= self.stay_threshold):

                            self.current_staying.add(track_id)

                            # 如果之前没有报警过，触发报警
                            if track_id not in self.alerted_objects and frame is not None:
                                self.alerted_objects.add(track_id)
                                self._trigger_alert(
                                    track_id, stationary_duration, (x1, y1, x2, y2), frame)

                        print(
                            f"🕒 轨迹ID {track_id} 静止时间: {stationary_duration:.1f}秒, 连续帧: {self.stationary_frames[track_id]}")

                    else:
                        # 目标在移动，重置静止状态（但保留报警记录）
                        self._reset_track_state(track_id, keep_alerted=True)
        ...
        self.staying_objects = self.current_staying.copy()
    ...
    def _reset_track_state(self, track_id, keep_alerted=False):
        """重置目标的停留状态"""
        ...
        if track_id in self.current_staying:
            self.current_staying.remove(track_id)
        # 如果不需要保留报警记录，也从alerted_objects中移除
        if not keep_alerted and track_id in self.alerted_objects:
            self.alerted_objects.remove(track_id)
    ...
    def get_alerted_objects(self):
        """获取曾经触发过报警的对象集合"""
        return self.alerted_objects.copy()